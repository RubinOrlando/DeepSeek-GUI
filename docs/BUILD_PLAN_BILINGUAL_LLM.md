...
**Resolved — Dutch-proficiency curriculum (this round):** the user proposed a structured "Dutch teacher" curriculum — progressive difficulty from fill-the-gap exercises, to full-sentence production with online-source-grounded correction (vocabulary, meaning, sentence logic), to long sentences/paragraphs, to full articles, with Bruges-model-conversation summarization as the ultimate comprehension test. This is sound curriculum-learning content design, not a new phase: the progressive-difficulty exercises feed Phase 1.1's corpus composition (1.1, new bullet) as CPT-stage content, and the correction/production/summarization stages become a new SFT slice, **2a.1d**, since SFT teaches task behavior in a language the model already has exposure to — it cannot substitute for CPT's raw-token-volume vocabulary/grammar acquisition if Phase 0.7's STRR gate comes back low. The summarization-as-final-test idea is kept as a qualitative comprehension probe (2a.5, Decision Gate) alongside, not instead of, the existing numeric Dutch thresholds.
...
2a.1d **Dutch-proficiency SFT slice (user-specified curriculum, this round):** a fourth distinct slice, not a variant of 2a.1b or 2a.1c — goal is general Dutch-language competence (vocabulary, grammar, sentence logic), not domain-specific behavior. Curated as a progressive-difficulty curriculum, reusing Phase 0.5's agentic search loop to ground corrections against online sources rather than trusting the SFT model's own unverified judgment of "correct Dutch":
   - **Stage 1 — fill-the-gap:** cloze-style exercises (missing word/conjugation/preposition), short context, single correct answer checkable against a reference. Cheapest stage to generate and verify at volume.
   - **Stage 2 — full-sentence production with correction:** given a prompt or partial context, produce a complete sentence; correct on three axes — vocabulary (wrong/imprecise word choice), meaning (sentence doesn't say what was intended), and sentence logic (grammatically valid but doesn't follow from context). Correction-pairs format, loss-masked the same way as 2a.1b/2a.1c.
   - **Stage 3 — long sentences and paragraphs:** same three-axis correction, applied at paragraph scale where sentence-to-sentence coherence becomes part of what's being checked, not just isolated-sentence grammar.
   - **Stage 4 — full articles:** comprehension and production at article length; this stage and Stage 3 need **seq=8192** (not the 2a.2 default of 2048) to avoid truncating full passages, same rationale as 2a.1b's seq choice.
   - **Stage 5 — Bruges-model-conversation summarization (ultimate test):** given a full Bruges-model session (2a.1b), produce a summary; graded on whether goal, exceptions, scale, and agreed next step all survive the summarization. This doubles as a domain-specific comprehension probe, not just a language-proficiency one — see 2a.5.

   **Native-curation requirement:** Stages 1–2 can be templated/generated at volume and spot-checked; Stages 3–5 need the same native-Dutch/Flemish curator review as 2a.1b/2a.1c/3.2, since correction at paragraph/article scale is a judgment call a template can't fully encode. **Scope caveat:** this slice teaches task behavior in a language the model already has pretraining-scale exposure to — it is not a substitute for Phase 1 CPT if Phase 0.7's STRR comes back low. SFT dataset sizes here are orders of magnitude smaller than CPT's 10–20B token budget; it cannot inject vocabulary/grammar the base model never saw at pretraining scale, only sharpen behavior around what's already there.
...
2a.5 Benchmark: MATH-500 held-out (100 samples) target 70%+, GSM8K (100) target 85%+, 20 Dutch math problems target 60–70%. **Decontaminated per Global Requirements.** **Counseling slice: spot-check 20 sample sessions for Bruges-model adherence (solution-focus maintained, no unsolicited advice-giving) — directional, not decision-grade, per Global Requirements' N<500 rule.** **Dutch-proficiency slice: spot-check N sample Stage-5 Bruges-conversation summaries (2a.1d) for goal/exceptions/scale/next-step preservation — directional, not decision-grade, per Global Requirements' N<500 rule; complements rather than replaces the Dutch comprehension numeric thresholds at the Decision Gate.**
...
- **Dutch comprehension check:** MMLU-Lite-nl (≥500 samples, decontaminated), XQuAD-nl, native-fluency spot-check. (2a.1d's Stage-5 Bruges-summarization spot-check, 2a.5, is a complementary qualitative signal here — it does not change these numeric thresholds.)
...
1.1 Corpus: 60–70% EN (FineWeb-Edu, SlimPajama reasoning-heavy, OpenWebMath) / 30–40% NL (Flemish/Dutch news, tech docs, forums, filtered) / 10–15% code. Target 10–20B tokens; quality over volume for NL — drop low-quality forum/web text rather than padding volume for size. **This sizing assumed full-model CPT's deeper per-token capacity (see Council correction in 1.3): if QLoRA ends up being the realistic path, reconsider whether 10–20B tokens is still right, or whether QLoRA's lower capacity-per-token argues for a larger, more heavily filtered corpus instead.** **Curriculum-graded NL content (user-specified, this round):** within the NL slice, include progressively-graded curriculum material — fill-the-gap exercises, corrected sentence/paragraph pairs, full articles — alongside generic scraped Dutch text; this is content composition within the existing token budget, not an additional corpus or phase. The correction/production/summarization side of the same curriculum (online-source-grounded vocabulary/meaning/logic correction) is SFT behavior, not CPT content — see 2a.1d. Store in Parquet: `[text, language, domain, quality_score]`.
